...
Every feature suggestion and hypothesis put through the bar — 29 verified, 17 unfinished, 27 declined. Typed in `config/hypotheses.tsv`; this table and the app's Patches page both render from it, so they cannot disagree.
...
| | Date | Area | Hypothesis | Verdict |
|---|---|---|---|---|
| 🟠 | 09-01 | engine | **Are the goal-expectation ratings better built from SHOTS ON TARGET than from goals? (the store already holds hst/ast; goals are the noisy realisation and shots regress better between matches)** | Measured and promising, not yet in the engine. scripts/shot_ratings.py replays both models as-of in one forward pass with an identical functional form, window, shrinkage and Poisson scoring — only the statistic differs — and scores them on U1.5/U2.5/U3.5/U4.5, four lines neither model selects. Over 40,113 fixtures in 8 leagues, shots alone beat goals alone by 0.48% of Brier, and a MIXTURE beats either: w=0.6 on shots gives +1.07%, with a flat optimum from 0.5 to 0.7 rather than a spike, and both time windows independently choosing a mixture and gaining about a point (+1.03% older, +1.16% newer). For scale, the whole gap to the bookmaker's closing line is 2.3%, so this is roughly half of it. CAVEAT that governs what happens next: the baseline here is a standalone goals model scoring 0.1925, while the real engine scores 0.1876 — it is already better than the thing shots beat, so the gain cannot be assumed to transfer. The next step is the shots signal inside the real pipeline, re-measured the same way. One bug found and fixed mid-experiment: an all-time cumulative conversion rate penalised only the shots model and reversed the whole result, turning +0.35% into -3.01% |
