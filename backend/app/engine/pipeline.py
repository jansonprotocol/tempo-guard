...
def translate_play(
    ...
    if lean == "over":
        ...
        if conf >= 0.82 and addon_ok:
            ...
            return TranslatedPlay(market="O2.5", confidence="LOW")
        # O2.25: requires strong signal — conf ≥ 0.78 AND p2p ≥ 0.80
        # Previously too loose (0.68/0.75) — was capturing borderline matches
        # that belong at O1.75. O2.25 hit rate was 40% vs O1.75 at 88%.
        if conf >= 0.78 and p2p_r >= 0.80:
            notes.append("Strong over signal → O2.25 (half win at 2 goals).")
            return TranslatedPlay(market="O2.25", confidence="MEDIUM")
        # O1.75: default safe floor for all other over calls — wins at 2+
