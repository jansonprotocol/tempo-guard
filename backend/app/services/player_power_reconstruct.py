...
def reconstruct_player_power_as_of(
    ...
    """
    # Get all match stats for this player before as_of_date
    ...
    # Use existing compute_player_power logic
    # This is a simplified version - you'll need to adapt your existing compute_player_power
    # to work with aggregated stats instead of PlayerSeasonStats objects
...
def _calculate_power_from_aggregates(stats: Dict, position: str) -> float:
    ...
    This should mirror your compute_player_power logic.
    """
    ...
    # Scale to 0-100 range (this should match your compute_player_power scaling)
    # This is a simplification - you may need to use z-scores like your original
    power = 50.0 + (avg_composite * 10)  # Adjust scaling factor as needed
...
def reconstruct_team_power_as_of(
    ...
    """
    # Get all players who played for this team before as_of_date
    ...
    # Reconstruct power for each player
    player_powers = []
    ...
    # Sort each group by minutes (or power) and take top slots
    zonal_scores = {}
    ...
    # Calculate overall squad power (weighted average)
    squad_power = 0.0
    weight_sum = 0.0
...
def _get_player_minutes_up_to(db: Session, player_id: int, as_of_date: date) -> int:
    """Get total minutes played by a player up to a given date."""
    result = db.query(db.func.sum(PlayerMatchStats.minutes)).filter(
        PlayerMatchStats.player_id == player_id,
        PlayerMatchStats.match_date < as_of_date
    ).scalar()
    return result or 0
